...
pub struct TriggerInspiratoryWidgetConfig<'a> {
    ...
    pub expiratory_trigger_container_parent: WidgetId,
    pub expiratory_trigger_text_widget: WidgetId,
    pub expiratory_trigger_less_button_widget: WidgetId,
    pub expiratory_trigger_more_button_widget: WidgetId,
    pub expiratory_trigger_value_widget: WidgetId,
...
impl<'a> ControlWidget<'a> {
    ...
    fn trigger_inspiratory_settings(&mut self, config: TriggerInspiratoryWidgetConfig) -> f64 {
        let sections_height = config.height / 4.0;
        ...
        widget::Rectangle::fill_with([config.width, sections_height], color::TRANSPARENT)
            ...
            .set(config.expiratory_trigger_container_parent, &mut self.ui);

        let mut expiratory_text_style = widget::text::Style::default();
        expiratory_text_style.font_id = Some(Some(self.fonts.regular));
        expiratory_text_style.color = Some(color::WHITE);
        expiratory_text_style.font_size = Some(20);

        widget::Text::new("Expiratory trigger")
            .with_style(expiratory_text_style)
            .top_left_of(config.expiratory_trigger_container_parent)
            .set(config.expiratory_trigger_text_widget, &mut self.ui);

        widget::Button::new()
            .right_from(config.expiratory_trigger_text_widget, 10.0)
            .w_h(30.0, 30.0)
            .label(&String::from("<-"))
            .set(config.expiratory_trigger_less_button_widget, &mut self.ui);

        let mut expiratory_value_style = widget::text::Style::default();
        expiratory_value_style.font_id = Some(Some(self.fonts.regular));
        expiratory_value_style.color = Some(color::WHITE);
        expiratory_value_style.font_size = Some(20);

        widget::Text::new(format!("{}", config.trigger_inspiratory_settings.expiratory_trigger).as_str())
            .with_style(expiratory_value_style)
            .right_from(config.expiratory_trigger_less_button_widget, 20.0)
            .set(config.expiratory_trigger_value_widget, &mut self.ui);

        widget::Button::new()
            .right_from(config.expiratory_trigger_value_widget, 20.0)
            .w_h(30.0, 30.0)
            .label(&String::from("->"))
            .set(config.expiratory_trigger_more_button_widget, &mut self.ui);

        widget::Rectangle::fill_with([config.width, sections_height], color::TRANSPARENT)
            .down_from(config.expiratory_trigger_container_parent, 0.0)
